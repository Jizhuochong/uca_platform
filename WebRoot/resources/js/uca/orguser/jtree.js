$(document).ready(function() {
	$('#tree_data').treegrid({
		url : rfPath + '/orgusertree/treeBy22?pid=0&oid=22',
		method : 'post',
		rownumbers : true,
		idField : 'id',
		treeField : 'name',
		...
		columns : [ [
		    ...
		    {field : 'operation',title : '操作',formatter : function(value, row, index) {
		    	...
		    	if(row.ty == '2') {
		    		return "<a href='javascript:toEdit(\""+ row.id + "\");' class='easyui-linkbutton'>激活用户</a>" +
...
//	    	$('#form_edit').form("clear");
	    	if(data.success) {
	    		...
	    		$('#user_jsdw_dialog').dialog('open').dialog('setTitle','激活用户');
...
function saveJsdwGrid(){
	...
	parent.$.messager.confirm('Confirm','是否确定激活该账户？',function(r){
      	if (r){
