...
<%@ include file="/WEB-INF/jsp/common/top.jsp" %>
	...
	<body>
	<div class="easyui-panel" title="创建资源" data-options="border:false,fit:true,closable:true,collapsible:true" style="padding: 20px;">
        <form name="formName">
        	<input type="hidden" name="resourceId" id="resourceId" value="0"/>
			<input type="hidden" name="resourceSort" id="resourceSort" value="0"/>
			<input type="hidden" name="resourceParentId" id="resourceParentId" value="${resourceParentId }"/>
			<input type="hidden" name="roleIds" id="roleIds" value=""/>
            <table cellpadding="9">
                <tr>
                    <td>资源名称：</td>
                    <td><input class="textbox" type="text" name="resourceName" id="resourceName" value="" size="60"></input></td>
                </tr>
                <tr>
                    <td>父级资源：</td>
                    <td><input class="textbox" type="text" id="resourceParentName" value="${resourceParentName }" size="60" readonly></input>&nbsp;&nbsp;<a id="menuBtn" href="javascript:void(0);" onclick="showMenu();">选择</a></td>
                </tr>
                <tr>
                    <td>资源链接：</td>
                    <td><input class="textbox" type="text" name="resourceUrl" id="resourceUrl" value="" size="60"></input></td>
                </tr>
                <tr>
                    <td>资源描述：</td>
                    <td><textarea name="resourceUrlDirection" id="resourceUrlDirection" rows="5" cols="56"></textarea></td>
                </tr>
                <tr>
                    <td>资源类型：</td>
                    <td>
                    	<input type="radio" name="resourceType" id="resourceType" value="1" checked="checked"/>URL&nbsp;&nbsp;
						<input type="radio" name="resourceType" id="resourceType" value="0"/>NO URL
					</td>
                </tr>
                <tr>
                    <td>资源状态：</td>
                    <td>
                    	<input type="radio" name="resourceStatus" id="resourceStatus" value="1" checked="checked"/>有效&nbsp;&nbsp;
						<input type="radio" name="resourceStatus" id="resourceStatus" value="0"/>无效
					</td>
                </tr>
                <tr>
                    <td>资源显示：</td>
                    <td>
                    	<input type="radio" name="resourceShowflag" id="resourceShowflag" value="1" checked="checked"/>显示&nbsp;&nbsp;
						<input type="radio" name="resourceShowflag" id="resourceShowflag" value="0"/>不显示
					</td>
                </tr>
                <tr>
                    <td>角色分配：</td>
                    <td>
                    	<c:forEach items="${roleList }" var="role" varStatus="urstatus">
							<input type="checkbox" name="roleId" onclick="javascript:checkroleids();" value="${role.roleId }" <c:forEach items="${resource.umRoles }" var="uRole"><c:if test="${uRole.roleId eq role.roleId }">checked</c:if></c:forEach>/><c:out value="${role.roleName }"></c:out>
						</c:forEach>
					</td>
                </tr>
                <tr>
