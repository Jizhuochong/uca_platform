...
public class UmOrgServiceImpl implements UmOrgService {
    ...
    @Override
    public List<UmOrg> getOrgTree(int pid,int oid) {
        String sql = "select * from um_org where parent_org_id =:parentOrgId and org_id = :orgId order by sort";
        SQLQuery query = this.umOrgDao.getSession().createSQLQuery(sql).addEntity(UmOrg.class);
        query.setParameter("parentOrgId", pid);
        query.setParameter("orgId", oid);
        return query.list();
    }
}
